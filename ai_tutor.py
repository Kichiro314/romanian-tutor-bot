...
Главная цель ученика: сдать собеседование с консулом Румынии на гражданство и достичь уровня B2.
...
Будь энергичным и поддерживающим. Никогда не унывай — делай язык доступным и весёлым.
Важно: всегда заканчивай мысль до конца, не обрывай предложения на середине."""
...
async def generate_daily_lesson(topic_id: str, topic_title: str, key_phrases: list = None) -> str:
    ...
    prompt = f"""Создай полноценный урок румынского языка по теме: "{topic_title}"

Цель: подготовить ученика к консульскому собеседованию и уровню B2.
{"Включи эти ключевые фразы:" + chr(10) + phrases_text if phrases_text else ""}

Формат (пройди ВСЕ пункты до конца, не обрывай):
1. Вступление с шуткой или мемом про румынский (2-3 предложения)
2. Топ-5 слов/фраз дня с примером из собеседования или реальной жизни
3. Грамматический момент — одно правило связанное с темой, кратко
4. Лайфхак как запомнить (ассоциация, мнемоника)
5. Мотивирующая фраза на румынском с переводом

Пиши ТОЛЬКО обычным текстом — без звёздочек, без подчёркиваний.
Каждый пункт доводи до логического конца."""

    return await _call(1500, prompt)
...
async def generate_fill_blank(recent_questions: list[str] = None) -> dict:
    ...
    prompt = f"""{avoid}Создай упражнение "вставь слово" для изучения румынского (уровень A1-B2).
...
Тема: консульское собеседование или повседневная жизнь и B2."""
...
async def generate_find_error(recent_questions: list[str] = None) -> dict:
    ...
    prompt = f"""{avoid}Создай упражнение "найди ошибку" для изучения румынского (уровень A1-B2).
...
Говори кратко (1-3 предложения), уровень A1-B2. Без звёздочек."""
...
async def generate_verb_of_day(learned_verbs: list[str] = None) -> dict:
    ...
    prompt = f"""{avoid}Дай один румынский глагол для изучения, уровень A1-B2.
Приоритет: глаголы для консульского собеседования или повседневной жизни на уровне B2.
